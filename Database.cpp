...
User Database::getProfile(const string &ID)
{
    return database[ID];
}
string Database::getEmail(const string &ID)
{
    return database[ID].email;
}
string Database::getFullName(const string &ID)
{
    return database[ID].fullName;
}
string Database::getPhoneNumber(const string &ID)
{
    return database[ID].phoneNumber;
}
string Database::getPassword(const string &ID)
{
...
{
    return (registeredEmails.count(email));
}

bool Database::isPhoneNumberExist(const string &phoneNumber)
{
    return (registeredPhoneNumbers.count(phoneNumber));
}
...
void Database::fetchProfileFromDatabase(User &profile, XLWorksheet &workSheet, uint const &index) const
{
    profile.rowID = index;
...
void Database::updateDatabaseMapAndSets(const string &ID, const string &infoWantedToChange, ucint &choice)
{
    ...
    if (choice == 2)
        ...
        updatePhoneNumber(infoWantedToChange, tempProfile);
    else if (choice == 4)
        updateEmail(infoWantedToChange, tempProfile);
    else if (choice == 5)
...
void Database::updateDatabaseFile(XLWorksheet &workSheet)
{
    ...
    {
        userProfile.second.rowID = rowID;
        rowID++;
